...
<%
    /*
     * HOW IT WORKS:
     * 1. Admin clicks "Add Staff" → AdminController → action="addStaff" → forwards here
     * 2. Admin fills form → action="addStaffSubmit" → adminDAO.addStaff()
...
<title>Add Staff – Menaka's ICL</title>
...
<style>
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  :root {
    ...
    --rose2:#c4706a; --rose2-dim:rgba(196,112,106,0.12);
  }
  ...
  .form-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(to right,var(--rose),transparent);}
  ...
  .form-group input,.form-group select{padding:11px 14px;background:var(--surface);border:1px solid var(--border);border-radius:10px;font-family:'Jost',sans-serif;font-size:14px;color:var(--text);outline:none;transition:border-color 0.2s,background 0.2s,box-shadow 0.2s;width:100%;}
  .form-group input::placeholder{color:rgba(255,255,255,0.2);}
  .form-group input:focus,.form-group select:focus{border-color:var(--rose);background:#1c1c1c;box-shadow:0 0 0 3px rgba(196,112,106,0.15);}
  .form-group select option{background:var(--surface);color:var(--text);}
  .btn-row{display:flex;gap:12px;margin-top:8px;}
  .btn-submit{flex:1;padding:13px;background:linear-gradient(135deg,#a85e5a,#7a3530);border:none;border-radius:10px;font-family:'Jost',sans-serif;font-size:12px;font-weight:600;letter-spacing:0.14em;text-transform:uppercase;color:#fff;cursor:pointer;box-shadow:0 4px 20px rgba(168,94,90,0.3);transition:transform 0.15s,filter 0.2s;}
...
<main>
  <div class="page-header">
    ...
    <h1 class="page-title">Add <em>Staff</em></h1>
    <p class="page-sub">Register a new staff member in the system</p>
  </div>
  <% if (message != null) { %>
  ...
  <div class="form-card">
    ...
    <form action="AdminController" method="post" autocomplete="off">
      <input type="hidden" name="action" value="addStaffSubmit">
      <div class="form-grid">
        ...
        <div class="form-group">
          <label>Age</label>
          <input type="number" name="age" placeholder="e.g. 35" min="18" max="65">
        </div>
        <div class="form-group">
          <label>Salary (Rs)</label>
          <input type="number" name="salary" placeholder="e.g. 45000" min="0" step="0.01">
        </div>
      </div>
      <div class="btn-row">

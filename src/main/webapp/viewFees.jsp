...
<%@ page import="com.adminModel.Fees, java.util.List" %>
<%
...
<%
    /*
     * HOW IT WORKS:
     * 1. Admin clicks "View Fees" → AdminController → action="viewFees"
     * 2. adminDAO.getAllFees() → forwards here with feeList
     */
    ...
    List<Fees> feeList = (List<Fees>) request.getAttribute("feeList");
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>View Fees – Menaka's ICL</title>
...
<style>
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .summary-bar{display:flex;align-items:center;gap:20px;background:var(--card);border:1px solid var(--border);border-radius:14px;padding:16px 24px;margin-bottom:20px;animation:fadeUp 0.5s 0.05s ease both;flex-wrap:wrap;}
  ...
  .summary-value{font-family:'Cormorant Garamond',serif;font-size:22px;font-weight:600;}
  ...
  .table-title{font-size:11px;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:var(--amber);display:flex;align-items:center;gap:10px;}
  .table-title::after{content:'';flex:1;height:0.5px;background:rgba(196,154,74,0.2);}
  ...
  .fee-id{font-family:monospace;font-size:11px;font-weight:600;color:var(--amber);background:var(--amber-dim);padding:3px 8px;border-radius:6px;}
  .roll-cell{font-family:monospace;font-size:12px;color:var(--mid);}
  .amount-cell{font-family:'Cormorant Garamond',serif;font-size:16px;font-weight:600;color:var(--gold);}
  .status-badge{display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:999px;font-size:11px;font-weight:600;letter-spacing:0.05em;}
  .status-badge::before{content:'';width:5px;height:5px;border-radius:50%;background:currentColor;}
  .status-badge.paid{background:var(--green-dim);color:var(--green);border:1px solid rgba(90,158,114,0.25);}
  .status-badge.pending{background:var(--rose-dim);color:var(--rose);border:1px solid rgba(196,112,106,0.25);}
  .tbl-btn{display:inline-flex;align-items:center;gap:4px;padding:5px 10px;border-radius:7px;font-family:'Jost',sans-serif;font-size:11px;font-weight:500;cursor:pointer;border:none;transition:background 0.18s;}
...
<main>
  <div class="page-header">
    ...
    <h1 class="page-title">View <em>Fees</em></h1>
    <p class="page-sub">All student fee payment records</p>
  </div>
  <%
    int total = 0; int paid = 0; int pending = 0; double totalAmt = 0;
    if (feeList != null) {
      total = feeList.size();
      for (Fees f : feeList) {
        totalAmt += f.getAmount();
        if ("PAID".equals(f.getStatus())) paid++;
        else pending++;
      }
    }
  %>
  <div class="summary-bar">
    <div class="summary-item">
      ...
      <div class="summary-value" style="color:var(--amber);"><%= total %></div>
    </div>
    <div class="summary-sep"></div>
    <div class="summary-item">
      <div class="summary-label">Total Amount</div>
      <div class="summary-value" style="color:var(--gold);">Rs. <%= String.format("%,.0f", totalAmt) %></div>
    </div>
    <div class="summary-sep"></div>
    <div class="summary-item">
      <div class="summary-label">Paid</div>
      <div class="summary-value" style="color:var(--green);"><%= paid %></div>
    </div>
    <div class="summary-sep"></div>
    <div class="summary-item">
      <div class="summary-label">Pending</div>
      <div class="summary-value" style="color:var(--rose);"><%= pending %></div>
    </div>
  </div>
  <div class="search-bar">
    <input type="text" id="searchInput" placeholder="Search by roll number or status..." onkeyup="filterTable()">
  </div>
  <div class="table-card">
    <div class="table-card-header">
      <div class="table-title">Fee Records</div>
    </div>
    ...
    <% } else { %>
      <table id="feeTable">
        ...
        <tbody>
          <% for (Fees f : feeList) { %>
          <tr>
            <td><span class="fee-id">#<%= f.getFeeId() %></span></td>
            <td><span class="roll-cell"><%= f.getRollNumber() %></span></td>
            <td><span class="amount-cell">Rs. <%= String.format("%,.2f", f.getAmount()) %></span></td>
            <td>
              <span class="status-badge <%= "PAID".equals(f.getStatus()) ? "paid" : "pending" %>">
                <%= f.getStatus() %>
              </span>
            </td>
            <td><%= f.getDueDate() %></td>
            <td>
              <form action="AdminController" method="post" style="margin:0;">
                ...
                <input type="hidden" name="fee_id" value="<%= f.getFeeId() %>">

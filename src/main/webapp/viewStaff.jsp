...
<%
    /*
     * HOW IT WORKS:
     * 1. Admin clicks "View Staff" → AdminController → action="viewStaff"
...
<title>View Staff – Menaka's ICL</title>
...
<style>
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .table-title{font-size:11px;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:var(--rose);display:flex;align-items:center;gap:10px;}
  .table-title::after{content:'';flex:1;height:0.5px;background:rgba(196,112,106,0.2);}
  ...
  .salary-cell{font-family:'Cormorant Garamond',serif;font-size:16px;font-weight:600;color:var(--green);}
  .att-cell{font-weight:600;color:var(--gold);}
  ...
  @media(max-width:820px){header{padding:0 20px;}main{padding:28px 20px 40px;}footer{padding:14px 20px;}thead th:nth-child(5),tbody td:nth-child(5){display:none;}}
</style>
</head>
<body>
<header>
...
<main>
  <div class="page-header">
    ...
    <h1 class="page-title">View <em>Staff</em></h1>
    <p class="page-sub">All staff members and their records</p>
  </div>
  <%
    int total = 0; int teaching = 0; int clerical = 0;
    if (staffList != null) {
      total = staffList.size();
      for (Staff s : staffList) {
        if ("Teaching".equals(s.getRole())) teaching++;
        else clerical++;
      }
    }
  %>
  <div class="summary-bar">
    <div class="summary-item">
      ...
      <div class="summary-value" style="color:var(--rose);"><%= total %></div>
    </div>
    <div class="summary-sep"></div>
    <div class="summary-item">
      <div class="summary-label">Teaching</div>
      <div class="summary-value" style="color:var(--blue);"><%= teaching %></div>
    </div>
    <div class="summary-sep"></div>
    <div class="summary-item">
      <div class="summary-label">Clerical</div>
      <div class="summary-value" style="color:var(--amber);"><%= clerical %></div>
    </div>
  </div>
  <div class="search-bar">
    <input type="text" id="searchInput" placeholder="Search by name, department or role..." onkeyup="filterTable()">
  </div>
      ...
      <table id="staffTable">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <span class="role-pill <%= s.getRole().toLowerCase() %>">
                <%= s.getRole() %>
              </span>
            </td>
            <td><%= s.getDepartment() %></td>
            <td><%= s.getAge() %></td>
            <td><span class="salary-cell">Rs. <%= String.format("%,.0f", s.getSalary()) %></span></td>
            <td><span class="att-cell"><%= s.getAttendance() %></span></td>
            <td>

...
<%
    /*
     * HOW IT WORKS:
     * 1. Admin clicks "Add Fee" → AdminController → action="addFee" → forwards here
     * 2. Admin fills form → action="addFeeSubmit" → adminDAO.addFee()
...
<title>Add Fee – Menaka's ICL</title>
...
<style>
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .form-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(to right,var(--amber),transparent);}
  .section-title{font-size:11px;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:var(--amber);display:flex;align-items:center;gap:10px;margin-bottom:20px;}
  ...
  .form-group input:focus,.form-group select:focus{border-color:var(--amber);background:#1c1c1c;box-shadow:0 0 0 3px rgba(196,154,74,0.15);}
  .form-group select option{background:var(--surface);color:var(--text);}
  .btn-row{display:flex;gap:12px;margin-top:8px;}
  .btn-submit{flex:1;padding:13px;background:linear-gradient(135deg,#b8873a,#8a6020);border:none;border-radius:10px;font-family:'Jost',sans-serif;font-size:12px;font-weight:600;letter-spacing:0.14em;text-transform:uppercase;color:#fff;cursor:pointer;box-shadow:0 4px 20px rgba(184,135,58,0.3);transition:transform 0.15s,filter 0.2s;}
...
<main>
  <div class="page-header">
    ...
    <h1 class="page-title">Add <em>Fee</em></h1>
    <p class="page-sub">Add a new fee record for a student</p>
  </div>
  <% if (message != null) { %>
  ...
  <div class="form-card">
    ...
    <form action="AdminController" method="post" autocomplete="off">
      <input type="hidden" name="action" value="addFeeSubmit">
      <div class="form-grid">
        <div class="form-group">
          <label>Roll Number</label>
          <input type="text" name="roll_number" placeholder="e.g. STU001" maxlength="10" required>
        </div>
        <div class="form-group">
          <label>Amount (Rs)</label>
          <input type="number" name="amount" placeholder="e.g. 15000" min="0" step="0.01" required>
        </div>
        <div class="form-group">
          <label>Status</label>

...
<%
    /*
     * HOW IT WORKS:
     * 1. Admin clicks "Delete Course" → AdminController → action="deleteCourse" (no id) → forwards here
     * 2. Admin enters course_id and confirms → action="deleteCourse" with id → adminDAO.deleteCourse()
     * 3. CASCADE removes Student_Course mappings automatically
     */
...
<title>Delete Course – Menaka's ICL</title>
...
<style>
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .page-eyebrow{font-size:11px;font-weight:500;letter-spacing:0.16em;text-transform:uppercase;color:var(--rose);display:flex;align-items:center;gap:10px;margin-bottom:10px;}
  .page-eyebrow::after{content:'';width:40px;height:0.5px;background:rgba(196,112,106,0.4);}
  ...
  .card{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:32px;max-width:500px;animation:fadeUp 0.5s 0.05s ease both;position:relative;overflow:hidden;}
  .card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(to right,var(--rose),transparent);}
  .section-title{font-size:11px;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:var(--rose);display:flex;align-items:center;gap:10px;margin-bottom:20px;}
  .section-title::after{content:'';flex:1;height:0.5px;background:rgba(196,112,106,0.2);}
  .warning-box{background:var(--rose-dim);border:1px solid rgba(196,112,106,0.2);border-radius:10px;padding:14px 16px;margin-bottom:22px;font-size:12px;color:var(--rose);line-height:1.7;display:flex;align-items:flex-start;gap:10px;}
  ...
  .form-group input{padding:11px 14px;background:var(--surface);border:1px solid var(--border);border-radius:10px;font-family:'Jost',sans-serif;font-size:14px;color:var(--text);outline:none;transition:border-color 0.2s,box-shadow 0.2s;}
  .form-group input::placeholder{color:rgba(255,255,255,0.2);}
  .form-group input:focus{border-color:var(--rose);background:#1c1c1c;box-shadow:0 0 0 3px rgba(196,112,106,0.15);}
  .confirm-row{display:flex;align-items:center;gap:10px;margin-bottom:22px;}
  .confirm-row input[type="checkbox"]{width:16px;height:16px;cursor:pointer;accent-color:var(--rose);}
  .confirm-row label{font-size:12px;color:var(--muted);cursor:pointer;}
  .btn-row{display:flex;gap:12px;}
  .btn-delete{flex:1;padding:13px;background:linear-gradient(135deg,#b03028,#7a1e18);border:none;border-radius:10px;font-family:'Jost',sans-serif;font-size:12px;font-weight:600;letter-spacing:0.14em;text-transform:uppercase;color:#fff;cursor:pointer;box-shadow:0 4px 20px rgba(176,48,40,0.3);transition:transform 0.15s,filter 0.2s;}
  .btn-delete:hover{transform:translateY(-1px);filter:brightness(1.1);}
  .btn-cancel{padding:13px 20px;background:transparent;border:1px solid var(--border-h);border-radius:10px;font-family:'Jost',sans-serif;font-size:12px;font-weight:500;letter-spacing:0.08em;text-transform:uppercase;color:var(--mid);cursor:pointer;text-decoration:none;display:flex;align-items:center;justify-content:center;transition:border-color 0.2s,color 0.2s;}
...
<main>
  <div class="page-header">
    ...
    <h1 class="page-title">Delete <em>Course</em></h1>
    <p class="page-sub">Permanently remove a course from the system</p>
  </div>
  <% if (message != null) { %>
  ...
  <% } %>
  <div class="card">
    <div class="section-title">Delete Course Record</div>
    <div class="warning-box">
      ...
      <span>This action is <strong>permanent.</strong> Deleting a course will also remove all student enrollments for this course automatically.</span>
    </div>
    <form action="AdminController" method="post" autocomplete="off" onsubmit="return confirmDelete()">
      <input type="hidden" name="action" value="deleteCourse">
      <div class="form-group">
        <label>Course ID</label>
        <input type="text" name="course_id" placeholder="e.g. C001" maxlength="10" required id="courseInput">
      </div>
      <div class="confirm-row">
        <input type="checkbox" id="confirmCheck">
        <label for="confirmCheck">I understand this will permanently delete the course</label>
      </div>
      <div class="btn-row">
...
<script>
  function confirmDelete() {
    ...
    const id = document.getElementById('courseInput').value.trim();
    if (!checkbox.checked) { alert('Please check the confirmation box.'); return false; }
    return confirm('Delete course: ' + id + '? This cannot be undone!');
  }
</script>
</body>
</html>
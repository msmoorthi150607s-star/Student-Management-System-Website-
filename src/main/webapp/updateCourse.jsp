...
<%@ page import="com.adminModel.Course" %>
<%
...
<%
    /*
     * HOW IT WORKS:
     * 1. Admin clicks "Update Course" → AdminController → action="updateCourse" → forwards here
     * 2. Admin searches by course_id → course loaded into form
     * 3. Admin edits and submits → action="updateCourseSubmit" → adminDAO.updateCourse()
     */
...
<title>Update Course – Menaka's ICL</title>
...
<style>
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .form-group input,.form-group textarea{padding:11px 14px;background:var(--surface);border:1px solid var(--border);border-radius:10px;font-family:'Jost',sans-serif;font-size:14px;color:var(--text);outline:none;transition:border-color 0.2s,box-shadow 0.2s;width:100%;}
  .form-group input::placeholder,.form-group textarea::placeholder{color:rgba(255,255,255,0.2);}
  .form-group input:focus,.form-group textarea:focus{border-color:var(--green);background:#1c1c1c;box-shadow:0 0 0 3px rgba(90,158,114,0.15);}
...
<main>
  <div class="page-header">
    ...
    <h1 class="page-title">Update <em>Course</em></h1>
    <p class="page-sub">Search by course ID then edit the details</p>
  </div>
  <% if (message != null) { %>
    ...
    <form action="AdminController" method="post" autocomplete="off">
      ...
      <div class="search-row">
        <div class="search-group">
          ...
          <input type="text" name="course_id" placeholder="e.g. C001" maxlength="10" required>
        </div>
  ...
  <% if (course != null) { %>
  <div class="form-card">
    <div class="section-title">Edit Course Details</div>
    <form action="AdminController" method="post" autocomplete="off">
      <input type="hidden" name="action" value="updateCourseSubmit">
      <div class="form-grid">
        <div class="form-group">
          <label>Course ID</label>
          <div class="readonly-field"><%= course.getCourseId() %></div>
          <input type="hidden" name="course_id" value="<%= course.getCourseId() %>">
        </div>
        <div class="form-group">
          <label>Course Name</label>
          <input type="text" name="course_name" value="<%= course.getCourseName() %>" required>
        </div>
        <div class="form-group">
          <label>Course Fee (Rs)</label>
          <input type="number" name="course_fee" value="<%= course.getCourseFee() %>" min="0" step="0.01" required>
        </div>
        <div class="form-group full">
          <label>Description</label>
          <textarea name="course_description"><%= course.getCourseDescription() %></textarea>
        </div>
      </div>
      <div class="btn-row">
